...
class CustomerMobileRegistry
{
     ...
     */
    public function assertUnique(array $entries, ?int $excludeCustomerId = null): void
    {
        $normalizedValues = [];
        ...
        foreach ($entries as $entry) {
            ...
            $exists = ContactMobileNumber::query()
                ...
                ->where('contactable_type', ContactMobileNumber::CONTACTABLE_CUSTOMER)
                ->when($excludeCustomerId !== null, function ($query) use ($excludeCustomerId): void {
                    $query->where('contactable_id', '!=', $excludeCustomerId);
                })
                ->exists();

            if ($exists) {
                throw ValidationException::withMessages([
                    'mobile_number' => __('This mobile number already exists for another customer.'),
                ]);
            }
        }
    }
     ...
     * @param  array<int, array{raw: string, category: string, contact_person_id?: int|null}>  $entries
     */
    public function syncForCustomer(int $customerId, array $entries): void
    {
