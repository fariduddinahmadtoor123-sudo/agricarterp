...
{
    public const SERVING_ROUTE = 'product-catalog.brand-images';

    public const SERVING_PANEL = 'admin';

    public function disk(): string
    {
        return (string) config('product-catalog.brand_logo_disk', 'local');
    }
    ...
    public function locate(mixed $rawPath): ?array
    {
        ...
        $candidates = array_values(array_unique(array_filter([
            $path,
            str_starts_with($path, 'brands/') ? null : 'brands/' . $path,
        ])));
    ...
    public function servingUrl(string $path): string
    {
        try {
            ...
            return url('/admin/brand-images?' . http_build_query(['path' => $path]));
        }
    }
